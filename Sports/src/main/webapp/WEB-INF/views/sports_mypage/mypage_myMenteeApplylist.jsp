...
<body>
					...
					<div class="article_container">
						<table style='min-width: 0px !important;' class="table table-borderless">
							<thead>
								<tr>
								  <th colspan="3">
								  	<h5>내 신청(멘티 글)리스트 <span id="writingcount">${listcount}</span></h5>
								  </th> 
								  <th colspan="2" style="text-align:right">
									<select id="mmSelect" class="form-control1">
          							  <option value="멘토" >멘토글 신청</option>
          							  <option value="멘티" selected>멘티글 신청</option>
                					</select>
								  </th>
								</tr>
								<tr>
							...
							<c:if test="${listcount > 0 }">
								<c:forEach var="apply" items="${applylist}">
									<tr>
			  							...
			  							<td><div>${apply.mentee_title}</div></td>
										<td colspan="2">신청 현황 : 
											<c:choose>
  ...
  <!-- 상세보기 modal -->
   <div class="modal" id="detailModal">
    ...
    <script src="${pageContext.request.contextPath}/resources/js/mmatch/menteeApplist.js"></script>
    <script>
	//상세정보 보기
