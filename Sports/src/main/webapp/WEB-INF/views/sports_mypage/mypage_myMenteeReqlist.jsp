...
<body>
    ...
    <div class="slider-area ">
        <!-- Mobile Menu -->
        <div class="single-slider slider-height2 d-flex align-items-center" data-background="${pageContext.request.contextPath}/resources/image/mmatch/mypage_mmreq.jpg">
            <div class="container">
                <div class="row">
                    <div class="col-xl-12">
					...
					<div class="article_container">
						<table style='min-width: 0px !important;' class="table table-borderless">
							<thead>
								<tr>
								  <th colspan="3">
								  	<h5>요청 (멘티 글)리스트 <span id="writingcount">${listcount}</span></h5>
								  </th> 
								  <th colspan="2" style="text-align:right">
									<select id="mmSelect" class="form-control1">
          							  <option value="멘토" >멘토글 요청</option>
          							  <option value="멘티" selected>멘티글 요청</option>
                					</select>
								  </th>
								</tr>
								<tr>
							...
							<c:if test="${listcount > 0 }">
								<c:forEach var="req" items="${reqlist}">
									<tr>
			  							<td><div>${req.sports_name}</div></td>
			  							<td><div>${req.mentee_title}</div></td>
										<td colspan="2">
											<span>${req.user_name}/${req.user_mobile}</span>
											<c:choose>
												<c:when test="${req.match_state == 1}">
												(요청대기)
												</c:when>
												<c:when test="${req.match_state == 2}">
												(요청수락)
												</c:when>
												<c:when test="${req.match_state == 3}">
												(요청거절)
												</c:when>
											</c:choose>
										</td>
			  							<td colspan="1" style="text-align: right !important;">
			  								<c:if test="${req.match_state == 1}">
				  								<button class="genric-btn info circle" onclick="javascript:respRequest(2,'${req.user_id}','${req.match_code}','${req.user_mobile}');">요청수락</button>
												<button class="genric-btn info circle" onclick="javascript:respRequest(3,'${req.user_id}','${req.match_code}','${req.user_mobile}');">요청거절</button> 
			  								</c:if>
											<button class="genric-btn info circle" onclick="javascript:detail('${req.match_code}');">상세보기</button>
										</td>	
									</tr>
								</c:forEach>	
							</c:if>
  ...
  <!-- 상세보기 modal -->
   <div class="modal" id="detailModal">
    ...
    <script src="${pageContext.request.contextPath}/resources/js/mmatch/menteeReqlist.js"></script>
    <script>
	//상세정보 보기
